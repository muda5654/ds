...
def main(page: Page):
    ...
    # Тип атаки
    type_attack = Dropdown(label='Тип атаки', hint_text='Выберите тип атаки', options=[dropdown.Option('Cмс+Звонки'), dropdown.Option('СМС'), dropdown.Option('ЗВОНКИ')], width=131, border_radius=40, alignment=alignment.bottom_center, border_color=color, value=check_config()['type_attack'], label_style=TextStyle(color=color), on_change=type_attack_change)
    ...
    def ADD():
        ...
        page.add(
                ...
                attack_button,
                Text('\n', size=12),
                Row([IconButton(icon='discord', icon_size=48, tooltip='Канал', url='https://discord.gg/Vr92CMCSPC', icon_color=color),
